...
- **每章必须有 `narrations.ts`**（与 `<Chapter>.tsx` 同目录）：
  ...
  - 每个元素 = 该 step 要播的口播文本（来自 `script.md` 对应段，
    **字字一致**——不是创意改写）
  - 视觉动画比口播长的 step 用 `{ text: "...", minHoldMs: 8000 }`
  - 这是**音频合成 + Auto 模式自动推进的唯一真相源**，写错或漏写
    会让录屏对不上嘴
  - 详细机制见 [`AUDIO.md`](AUDIO.md)
...
- [ ] **每条 narration 文本与 `script.md` 对应段落字字一致**（不是改写、
      不是摘要）—— 录屏的画外音必须和稿子相同
- [ ] 任何视觉动画明显长于口播的 step，narrations 该条用
      `{ text, minHoldMs: <动画毫秒数> }` 形式，否则 Auto 模式会切断动画
